    ...
    d_trellis_size(trellis_size)
{
    ...
    auto enco_n = aff3ct::module::Encoder_RSC_generic_sys<>(frame_size, N_rsc, true, {013, 015});
    auto enco_i = enco_n;

    auto trellis_n = enco_n.get_trellis();
    auto trellis_i = trellis_n;
    
    auto dec_n = aff3ct::module::Decoder_RSC_BCJR_seq_fast<>(frame_size, trellis_n, buffered);
    auto dec_i = aff3ct::module::Decoder_RSC_BCJR_seq_fast<>(frame_size, trellis_i, buffered);

    d_interleaver_core = std::make_shared<aff3ct::tools::Interleaver_core_LTE<>>(frame_size);
    auto pi = std::make_shared<aff3ct::module::Interleaver<float>>(*d_interleaver_core);

    d_decoder = std::unique_ptr<aff3ct::module::Decoder_turbo_fast<>>(new aff3ct::module::Decoder_turbo_fast<>(frame_size, d_input_size, n_iterations, dec_n, dec_i, *pi, buffered));
...
void turbo_decoder_impl::generic_work(const void* inbuffer, void* outbuffer)
{
    ...
    d_decoder->decode_siho(in, out, -1);
