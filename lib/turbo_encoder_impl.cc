...
namespace gr {
  namespace fec_dev {
    ...
    generic_encoder::sptr turbo_encoder::make(int frame_size, bool pack, bool packed_bits)
    {
        return generic_encoder::sptr(new turbo_encoder_impl(frame_size, pack, packed_bits));
    }
    ...
    bool turbo_encoder_impl::set_frame_size(unsigned int frame_size)
    {
        ...
        if (frame_size > d_max_frame_size) {
            d_logger->info("tried to set frame to {:d}; max possible is {:d}",*9
                          frame_size,
                          d_max_frame_size);
