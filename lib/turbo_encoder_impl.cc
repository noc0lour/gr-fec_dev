                                               ...
                                               int trellis_size)
{
        ...
        d_trellis_size(trellis_size)
    {
        ...
        auto enco_n = aff3ct::module::Encoder_RSC_generic_sys<B_8>(d_frame_size, N_rsc, buffered, polys);
        auto enco_i = enco_n;
        
        auto interleaver_core = aff3ct::tools::Interleaver_core_LTE<>(frame_size);
        auto pi = aff3ct::module::Interleaver<B_8>(interleaver_core);

        std::cout << "Creating d_encoder" << std::endl;
        d_encoder = std::make_unique<aff3ct::module::Encoder_turbo<B_8>>(frame_size, d_output_size, enco_n, enco_i, pi);
